...
const Profile2Model = require('../models/Profile2/Profile2Model');
const SelectedMenu2Model =require('../models/Profile2/SelectedMenu2')
...
router.post('/create', (req, res) => {
  ...
  Profile2Model.create({ name, description, price, imgUrl })
...
router.put('/update/:id', (req, res)=> {
  const id = req.params.id;
  Profile2Model.findByIdAndUpdate({_id:id},{name:req.body.name, description:req.body.description, price:req.body.price, imgUrl:req.body.imgUrl})
...
router.delete('/delete/:id',(req,res) => {
  const id = req.params.id;
  Profile2Model.findOneAndDelete({_id:id})
...
router.post('/saveSelectedFood', (req, res) => {
  ...
  SelectedMenu2Model.create({ name, description, price, imgUrl })
    .then((selectedFood) => {
...
router.delete('/deleteSelectedFood/:id', (req, res) => {
  ...
  SelectedMenu2Model.findByIdAndDelete(id)
    .then(deletedItem => {
      if (!deletedItem) {
...
router.delete('/clearTable', (req, res) => {
  SelectedMenu2Model.deleteMany() // You missed the parentheses here
    .then(deletedItems => {
